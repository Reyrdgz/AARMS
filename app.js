...
const PARAMS_TOMA=['FQ','TOC','Hg','MP','CIAN','FOS.','SAAM','GYA','DQO','DBO5','N.TOT','CTYF','ENTE.','NO2','NO3','H.HELM','COLOR','E.COLI','TOX.AGUDA','CLORUROS','CR.HEX.','OTROS'];
const CONS_O=[['1','H₂SO₄'],['2','NaOH'],['3','K₂Cr₂O₇ 25%'],['4','Hielo 4°C'],['5','NA'],['6','HNO₃'],['7','Bolsa c/tios.'],['8','HCl'],['9','HNO₃ Sup.'],['10','H₂SO₄ 25%'],['11','Bolsa s/tios.'],['12','Buffer'],['13','Formald.'],['14','Otro']];
...
const VOLS={FQ:4000,TOC:1000,Hg:500,MP:500,CIAN:1000,'FOS.':500,SAAM:1000,GYA:1000,DQO:500,DBO5:1000,'N.TOT':2000,CTYF:100,'ENTE.':250,NO2:500,NO3:500,'H.HELM':5000,COLOR:250,'E.COLI':100,'TOX.AGUDA':40,CLORUROS:500,'CR.HEX.':500,OTROS:500};
...
const CADENA_DATA={
  ...
  'H.HELM':{pres:'13', vol:'5000', env:'5',  ph:'N/A'},
  'COLOR': {pres:'4',  vol:'250',  env:'11', ph:'N/A'},
  'E.COLI':{pres:'7',  vol:'100',  env:'9',  ph:'N/A'},
  'TOX.AG.':{pres:'4', vol:'40',   env:'10', ph:'N/A'},
  'CLORU.':{pres:'4',  vol:'500',  env:'4',  ph:'N/A'},
  'CR.HEX.':{pres:'4', vol:'250',  env:'11', ph:'N/A'},
  'TOX.AGUDA':{pres:'4',vol:'40', env:'10', ph:'N/A'},
  'CLORUROS':{pres:'4', vol:'500', env:'4',  ph:'N/A'},
};
const CADENA_SIMPLES=['GYA','CTYF','ENTE.','E.COLI','TOX.AGUDA'];

function buildCusTable(){
  // Active params from tomas
  const activeSet=new Set([
    ...tomas.flatMap(t=>[...t.params]),
    ...CADENA_SIMPLES
  ]);
  const allParams=['FQ','TOC','Hg','MP','CIAN','FOS.','SAAM','GYA','DQO','DBO5','N.TOT','CTYF','ENTE.','NO2','NO3','H.HELM','COLOR','E.COLI','TOX.AGUDA','CLORUROS','CR.HEX.'];
  ...
  document.getElementById('ctHead').innerHTML=
    ...
    allParams.map(function(p){
      ...
      var bg=active?(isS?'#d2eee8':'#e8eef5'):'#f0f2f5';
      var tc=active?(isS?'#007a60':'#0a1628'):'#aaa';
      return '<th style="background:'+bg+';color:'+tc+'">'+p+'</th>';
    }).join('');
  ...
  document.getElementById('ctBody').innerHTML=ROWS.map(function(rowDef,ri){
    ...
    var cells=allParams.map(function(p,ci){
      ...
      var id='cc_'+ri+'_'+ci;
      if(!active){
        return '<td style="background:#e4e8f0;position:relative;overflow:hidden"><div style="position:absolute;inset:0;background:repeating-linear-gradient(135deg,transparent,transparent 4px,rgba(0,0,0,.06) 4px,rgba(0,0,0,.06) 5px)"></div></td>';
      }
      if(ri>=5){
        var bg=isS?'#e8f5f0':'#fff';
        return '<td><input id="'+id+'" type="text" style="width:100%;border:none;background:'+bg+';text-align:center;font-size:11px;padding:2px" placeholder="—"></td>';
      }
      var bg2=isS?'#d2eee8':'#fff';
      var tc=isS?'#007a60':'#334155';
      return '<td style="text-align:center;font-weight:700;font-size:12px;background:'+bg2+';color:'+tc+'">'+val+'</td>';
    });
    return '<tr><td class="stl" style="font-weight:700;font-size:11px">'+lbl+'</td>'+cells.join('')+'</tr>';
  }).join('');
}
  ...
  modal.innerHTML = `
    ...
    <div style="flex:1;overflow-y:auto;padding:16px">
      ...
      ` : lista.map((m,i) => {
        ...
        return `
          <div style="background:var(--bg2);border:1px solid var(--ln2);border-radius:12px;padding:14px 16px;margin-bottom:10px;cursor:pointer" onclick="cargarMuestreo(${m.id})">
...
async function buildPDF(){
  ...
  const [imgFoto,imgSigCli,imgSigMuest]=await Promise.all([
  ...
  // ── HEADER ──
  // Header total height = 72pt — logo centered vertically with breathing room
  const HDR=72;

  // Logo — free, no box, no border
  doc.addImage(LOGO_PDF_URI,'PNG',M+4,6,84,63,'','FAST');

  // Company info — starts at x=M+100, vertically centered in 72pt
  doc.setTextColor(...NAVY);doc.setFont('helvetica','bold');doc.setFontSize(13);
  doc.text('ASESORÍA Y ANÁLISIS S.C.',W/2-50,22,{align:'center'});
  doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(...BLUE);
  doc.text('Laboratorio de Alimentos y Aguas',W/2-50,32,{align:'center'});
  doc.setTextColor(100,110,130);doc.setFontSize(7);
  doc.text('Calle 12 Ave. Serdán Ext. 465 Int. 201  |  Edif. Puertas del Sol  |  Col. Centro C.P. 85400',W/2-50,42,{align:'center'});
  doc.text('Tel: 622 224 0910  FAX 622 224 207',W/2-50,51,{align:'center'});
  ...
  folios.forEach(([l,v],i)=>{
    ...
    doc.text(v||'',W-77,fy+12);
    if(i<2){doc.setDrawColor(...MGRAY);doc.setLineWidth(0.3);doc.line(W-80,fy+15,W-M,fy+15);}
  });
  ...
  const PARAMS=['FQ','TOC','Hg','MP','CIAN','FOS.','SAAM','GYA','DQO','DBO5','N.TOT','CTYF','ENTE.','NO2','NO3','H.HELM','COLOR','E.COLI','TOX.AGUDA','CLORUROS','CR.HEX.','OTROS'];
  const SIMPLES=['GYA','CTYF','ENTE.','E.COLI','TOX.AGUDA'];
  const fixW=[26,34,22,22,28,30,30,38,34,28,26,26]; // 12 cols = 344
  const fixSum=fixW.reduce((a,b)=>a+b,0);
  const pW=Math.floor((CW-fixSum)/PARAMS.length); // ~18.4
  const fixHdrs=['MUESTRA','HORA','%VOL','L/s','T.AMB','T.AGUA','pH25°C','MAT.FL','COND.µS','COLOR','OLOR','CLORO'];
  const th=10,th2=9;

  // SINGLE unified header row — fixed cols + param cols all at same height
  const thH=13; // single header row height, tall enough for all labels
  let x=M;
  // Fixed columns
  fixHdrs.forEach((h,i)=>{
    ...
    doc.setTextColor(26,58,107);doc.setFont('helvetica','bold');doc.setFontSize(5.5);
    doc.text(h,x+fixW[i]/2,y+thH/2+2,{align:'center'});
    x+=fixW[i];
  });
  // Param columns
  PARAMS.forEach((p,i)=>{
    ...
    doc.text(p,x+pW/2,y+thH/2+2,{align:'center'});
    x+=pW;
  });
  y+=thH;
  ...
  allTomas.forEach((t,idx)=>{
    ...
    if(t){
      ...
      // Empty row
      doc.setFillColor(...bg);
      doc.rect(M,y,CW,tomaH,'F');
      doc.setDrawColor(220,228,236);doc.setLineWidth(0.2);
      let ex=M;
      [...fixW,...PARAMS.map(()=>pW)].forEach(w=>{doc.rect(ex,y,w,tomaH,'S');ex+=w;});
      // T label
      doc.setFillColor(220,226,234);
      doc.rect(M,y,fixW[0],tomaH,'F');
      doc.setTextColor(160,170,185);doc.setFont('helvetica','bold');doc.setFontSize(7);
      doc.text(`T${idx+1}`,M+fixW[0]/2,y+8,{align:'center'});
    }
    y+=tomaH;
  });
  ...
  const FRASCO_MAP={
    'FQ':3,        // Frasco Plástico 4Lt
    'TOC':8,       // Frasco Vidrio Ámbar 1Lt
    'Hg':4,        // Frasco Plástico 500mL
    'MP':4,        // Frasco Plástico 500mL
    'CIAN':4,      // Frasco Plástico 500mL
    'FOS.':4,      // Frasco Plástico 500mL
    'SAAM':2,      // Frasco Plástico 1Lt
    'GYA':1,       // Frasco boca ancha vidrio 1Lt
    'DQO':4,       // Frasco Plástico 500mL
    'DBO5':2,      // Frasco Plástico 1Lt
    'N.TOT':13,    // Frasco Plástico 2Lt
    'CTYF':6,      // Bolsa Estéril 300mL
    'ENTE.':6,     // Bolsa Estéril 300mL
    'NO2':4,       // Frasco Plástico 500mL
    'NO3':4,       // Frasco Plástico 500mL
    'H.HELM':5,    // Frasco Plástico 5Lt
    'COLOR':11,    // Frasco Vidrio Ámbar 250mL
    'E.COLI':7,    // Bolsa Estéril 300mL c/Tiosulfato
    'TOX.AG.':10,  // Frasco Vidrio Ámbar 40mL
    'CLORU.':4,    // Frasco Plástico 500mL
    'CR.HEX.':11,  // Frasco Vidrio Ámbar 250mL
  };
  const tipoH=10;
  ...
  PARAMS.forEach(p=>{
    ...
    const hasP=tomas.some(t=>t.params.has(p))||SIMPLES.includes(p);
    if(hasP){
  ...
  const infoItems=[
    ...
    ['Fecha final:',gv('h_fin').replace('T',' ')],
  ...
  infoItems.forEach(([l,v],i)=>{
    const iy=y+26+i*16; // 16pt per row: label 7pt + value 8pt = 15pt total
  ...
  doc.addImage(LOGO_PDF_URI,'PNG',M+2,2,36,27,'','FAST');
  doc.setTextColor(...WHITE);doc.setFont('helvetica','bold');doc.setFontSize(10);
...
async function buildPDFCliente(){
  ...
  const [imgFoto2,imgSig,imgSig2]=await Promise.all([
  ...
  // ════════ HEADER ════════
  const HH=52,LW=88,MW=138,TW2=CW-LW-MW;
  outerBorder(ML,y,CW,HH);
  innerLine(ML+LW,y,ML+LW,y+HH);
  innerLine(ML+LW+TW2,y,ML+LW+TW2,y+HH);

  // Logo — ratio 1080x815 = 1.325:1, render at 70x53 to fit cleanly
  try{doc.addImage(LOGO_PDF_URI,'PNG',ML+3,y+2,82,62,'','FAST');}catch(e){}
  ...
  // Map/info box (RIGHT)
  const rX=ML+FW;
  // Try to add a static map image using OpenStreetMap tile
  const lat=parseFloat(gv('gps_n')), lng=parseFloat(gv('gps_w'));
  if(!isNaN(lat)&&!isNaN(lng)){
    // Static map placeholder — show info grid instead since we can't fetch in jsPDF
    doc.setFillColor(236,240,248);doc.rect(rX,y,FW,FH,'F');
    doc.setDrawColor(...K.bdrCell);doc.setLineWidth(0.3);doc.rect(rX,y,FW,FH,'S');
    // Draw a simple map-like visual
    doc.setTextColor(...K.lbl);doc.setFont('helvetica','bold');doc.setFontSize(7);
    ...
    doc.setFillColor(...K.wht);doc.circle(pinX,pinY,2.5,'F');
    // Coords label
    doc.setTextColor(...K.navy);doc.setFont('courier','bold');doc.setFontSize(8);
    doc.text(lat.toFixed(4)+'° N',rX+FW/2,y+FH-22,{align:'center'});
    doc.text(lng.toFixed(4)+'° W',rX+FW/2,y+FH-12,{align:'center'});
  }else{
    // Info list fallback
    ...
    const infoList=[
      ...
      ['Tipo',gv('h_tipo')],['Coord. N',gv('gps_n')||'—'],['Coord. W',gv('gps_w')||'—'],
    ];
    infoList.forEach(([l,v],i)=>{
      const iy=y+14+i*16;
      if(i>0){cellLine(rX+4,iy-1,rX+FW-4,iy-1);}
  ...
  const CMAP={
    ...
    'NO3':'H2SO4','H.HELM':'Formaldehido','COLOR':'N/A',
    'E.COLI':'Bolsa c/Tios.','TOX.AG.':'Bolsa','CLORU.':'N/A','CR.HEX.':'N/A'
  };
  const SIMPLES=['GYA','CTYF','ENTE.','E.COLI','TOX.AGUDA'];
  const allP=[...new Set([...tomas.flatMap(t=>[...t.params]),...SIMPLES])];
  ...
  allP.slice(0,12).forEach((p,ri)=>{
    ...
    for(let ti=0;ti<6;ti++){
      const cx=ML+CLW+CCW+ti*CTW;
      const t=tomas[ti];
      const hasP=t&&(t.params.has(p)||SIMPLES.includes(p));
...
async function buildPDFCadena(){
  ...
  const DATA={
    ...
    'TOX.AGUDA':{pres:'4',vol:'40',env:'10',ph:'N/A'},
    'CLORUROS':{pres:'4',vol:'500',env:'4',ph:'N/A'},
    'OTROS':{pres:'',vol:'',env:'',ph:''},
  };

  // Active params from tomas
  const activeSet=new Set([...tomas.flatMap(t=>[...t.params]),...SIMPLES]);
  ...
  const TH=12; // param header height
  ...
  // Logo
  try{doc.addImage(LOGO_PDF_URI,'PNG',M,y,48,36,'','FAST');}catch(e){}

  // Company
  doc.setTextColor(...NAVY);doc.setFont('helvetica','bold');doc.setFontSize(7);
  doc.text('Asesoría y Análisis, S.C.',M+52,y+9);
  doc.setFont('helvetica','normal');doc.setFontSize(5.5);doc.setTextColor(...DGRAY);
  doc.text('Calle 12 Ave. Serdán Ext.465 Int.201, Edif. Puertas del Sol, Col. Centro C.P. 85400  Tel: 622 224 0910',M+52,y+17);

  // Title centered
  doc.setTextColor(...NAVY);doc.setFont('helvetica','bold');doc.setFontSize(11);
  doc.text('CADENA DE CUSTODIA INTERNA AR',W/2,y+14,{align:'center'});
  doc.setFont('helvetica','normal');doc.setFontSize(5.5);doc.setTextColor(...MGRAY);
  doc.text('F-AA-01A-15',W/2,y+22,{align:'center'});

  // Folios right
  const fX=W-M-110;
  doc.setDrawColor(...MGRAY);doc.setLineWidth(0.4);
  doc.line(fX,y,fX,y+36);
  [['FOLIO CCIAR:',gv('h_cciar')],['FOLIO OMAR:',gv('h_omar')],['FOLIO HCAR:',gv('h_hcar')]].forEach(([l,v],i)=>{
    ...
    doc.text(l,fX+4,y+6+i*11);
    doc.setFont('helvetica','normal');doc.setTextColor(...DGRAY);doc.setFontSize(6.5);
    doc.text(v||'__________',fX+38,y+6+i*11);
  });
  y+=40;

  // ── IDENTIFICACIÓN ──
  fillRect(M,y,CW,10,LGRAY);
  outerRect(M,y,CW,10);
  doc.setTextColor(...NAVY);doc.setFont('helvetica','bold');doc.setFontSize(6);
  doc.text('IDENTIFICACIÓN DE LA MUESTRA:',M+3,y+7);
  doc.setFont('helvetica','normal');doc.setTextColor(...DGRAY);doc.setFontSize(6.5);
  doc.text(gv('h_idm')||'___________________________',M+75,y+7);
  doc.setTextColor(...NAVY);doc.setFont('helvetica','bold');doc.setFontSize(6);
  doc.text('TIPO:',M+250,y+7);
  doc.setFont('helvetica','normal');doc.setTextColor(...DGRAY);
  doc.text(gv('h_tipo')||'____________',M+265,y+7);
  doc.setTextColor(...NAVY);doc.setFont('helvetica','bold');
  doc.text('TIEMPOS:',M+370,y+7);
  doc.setFont('helvetica','normal');doc.setTextColor(...DGRAY);
  doc.text(gv('h_int')||'_______',M+400,y+7);
  doc.setTextColor(...NAVY);doc.setFont('helvetica','bold');
  doc.text('TRANSPORTA EN HIELERA: □  OTRO:',M+460,y+7);
  y+=10;
  ...
  doc.text('PARÁMETROS',M+LABEL_W/2,y+8.5,{align:'center'});
  ...
  PARAMS.forEach(p=>{
    ...
    doc.text(p,px+PCOL/2,y+8.5,{align:'center'});
    px+=PCOL;
  });
  ...
  doc.text('PRESERVACIÓN:',M+3,y+6.5);
  doc.setFont('helvetica','normal');doc.setFontSize(4.5);
  doc.text('1-H₂SO₄  2-NaOH  3-K₂Cr₂O₇25%  4-Hielo4°C  5-NA  6-HNO₃  7-Bolsas c/Tiosulfato  8-HCl  9-HNO₃Sup  10-H₂SO₄25%  11-Bolsas s/Tiosulfato  12-Dil.Buffer  13-Formaldehído10%  14-Otro:_____',M+38,y+6.5);
  doc.setFont('helvetica','bold');doc.text('TIPO ENVASE:',M+3,y+14);
  doc.setFont('helvetica','normal');
  doc.text('1-V.Ancho1L  2-Plást.1L  3-Plást.4L  4-Plást.500mL  5-Plást.5L  6-B.Est.300mL  7-B.Est.300mL c/Tios.  8-V.Ámb.1L  9-B.Est.100mL c/Tios.  10-V.Ámb.40mL  11-V.Ámb.250mL  12-B.Est.100mL  13-Plást.2L',M+35,y+14);
  y+=notaH+2;
